...
public class DataService {
	public static boolean add_mdmunitsByCount(MDMUNITS[] data,String table) {
		...
		try {
	    	...
	    	for (int i=1,k=0; i <= count; i++) {
				if(i%1000==0 || i==count){
					for (MDMUNITS d : Arrays.asList(data).subList(1000*k, i)){
	...
	public static boolean add_mdmmatsortsByCount(MDMMATSORTS[] data,String table) {
		...
		try {
			...
			for (int i=1,k=0; i <= count; i++) {
				if(i%1000==0 || i==count){
				for (MDMMATSORTS d : Arrays.asList(data).subList(1000*k, i)){
	...
	public static boolean add_mdmmaterialsByCount(MDMMATERIALS[] data,String table) {
		...
		try {
			...
			for (int i=1,k=0; i <= count; i++) {
				if(i%1000==0 || i==count){
				for (MDMMATERIALS d : Arrays.asList(data).subList(1000*k, i)){
	...
	public static boolean add_mdmmatcomByCount(MDMMATCOM[] data,String table) {
		...
		try {
			...
			for (int i=1,k=0; i <= count; i++) {
				if(i%1000==0 || i==count){
				for (MDMMATCOM d : Arrays.asList(data).subList(1000*k, i)){
